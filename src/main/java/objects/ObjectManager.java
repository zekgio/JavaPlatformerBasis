...
public class ObjectManager {
	...
	public void applyEffectToPlayer(Potion p) {
		switch (p.getObjType()){
			...
			case PURPLE_POTION -> playing.getPlayer().setCanDoubleJump(true);
			case BLUE_POTION ->   playing.getPlayer().changePower(BLUE_POTION_VALUE);
			case YELLOW_POTION -> playing.getPlayer().changePower(YELLOW_POTION_VALUE);
		}
	}
					...
					if (isPlayerInRange(c, player))
						if (isPlayerInfrontOfCannon(c, player))
							if (CanCannonSeePlayer(lvlData, player.getHitbox(), c.getHitbox(), c.getTileY()))
								c.setAnimation(true);
			c.update();
			if (c.getAniIndex() == 4 && c.getAniTick() == 0) shootCannon(c);
		}
	}

	private void shootCannon(Cannon c) {
		int dir = 1;
		if (c.getObjType() == CANNON_LEFT) dir = -1;
	...
	private void drawPotions(Graphics g, int xLvlOffset, int yLvlOffset) {
		for (Potion p : potions)
			if (p.isActive()) {
				...
				else if(p.getObjType()>1) type = p.getObjType()-5;
